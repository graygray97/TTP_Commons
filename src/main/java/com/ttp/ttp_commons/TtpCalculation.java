...
public class TtpCalculation {
    ...
    public static List<Path> runSolutions(String[] locationList, String apiKey, String mode) throws IOException, ApiException, InterruptedException {
    ...
    private static List<Path> getPaths(List<Location> locations, GeoApiContext mGeoApicontext, String mode) throws ApiException, InterruptedException, IOException {
        List<Path> paths = new ArrayList<>();
        for(int i = 0; i < locations.size(); i++ ){
            for(int j = 0; j < locations.size(); j++){
                if(!(i == j)){
                    Path path = new Path(locations.get(i), locations.get(j));
                    path.CalculateDirections(mGeoApicontext, mode, true);
                    paths.add(path);
                }
            }
        }
    ...
    private static List<Location> getLocations(String[] locationList) {
        List<Location> locations = new ArrayList<>();
        for(int i = 0; i < locationList.length; i++){
            String[] location = locationList[i].split(",");
            locations.add(new Location(new LatLng(Double.parseDouble(location[0]), Double.parseDouble(location[1])), (i == 0)));
        }
        return locations;
    }
